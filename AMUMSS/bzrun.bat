...
REM if %_bCPU% gtr %_bMinCPU% (
	REM set _bAllowMapFileTreeCreator=N
	REM set _bRecreateMapFileTrees=1
REM )
...
if %_bOS_bitness%==64 (
	...
	REM xcopy /s /y /h /v ".\MODBUILDER\Extras\lua-lfs.5.3.5_x64\*.*" ".\MODBUILDER\*" 1>NUL 2>NUL
) else (
	REM need to bring in lua_x86
	REM xcopy /s /y /h /v ".\MODBUILDER\Extras\lua-lfs.5.3.5_x86\*.*" ".\MODBUILDER\*" 1>NUL 2>NUL
)

rem set "_mLUA=lua-lfs.exe"
				...
				if !_uOldMBINCompilerFlag!==N (
					...
					FOR /r "%~dp0\ModScript\EXTRACTED_PAK" %%G in (*.lua) do ( 
						...
						if not defined _UseLuaInPak (
							REM CHOICE /c:YN /m "%_zRED%??? Do you want to rebuild the MOD pak(s) using this script %_zDEFAULT%"
							REM if !ERRORLEVEL! EQU 2 SET _bUseLuaInPak=N
							REM if !ERRORLEVEL! EQU 1 SET _bUseLuaInPak=Y
							SET _bUseLuaInPak=Y
						)
...
REM CHOICE /c:yn /m "%_zRED%??? Do you want to create a COMBINED mod[Y] or INDIVIDUAL mod(s)[N] %_zDEFAULT%"
REM if %ERRORLEVEL% EQU 2 goto :CONTINUE_EXECUTION1
...
echo.               ...in this case
REM CHOICE /c:yn /m "%_zRED%??? Do you want to use a COMPOSITE combined MOD named just like that %_zDEFAULT%"
REM if %ERRORLEVEL% EQU 2 goto :COMBINEDTYPE
REM if %ERRORLEVEL% EQU 1 SET _bCOMBINE_MODS=3
...
echo.
REM CHOICE /c:yn /m "%_zRED%??? Do you want to use a NUMERIC suffix[Y] or the current DATE-TIME[N] to differentiate your mod name %_zDEFAULT%"
REM if %ERRORLEVEL% EQU 2 SET _bCOMBINE_MODS=1
REM if %ERRORLEVEL% EQU 1 SET _bCOMBINE_MODS=2
SET _bCOMBINE_MODS=2
goto :SIMPLE_MODE

:CONTINUE_EXECUTION1
REM echo.
REM CHOICE /c:NSA /m "%_zRED%??? Would you like or [N]ot to COPY [S]ome or [A]ll Created Mod PAKs to your game folder and DELETE [DISABLEMODS.TXT] %_zDEFAULT%"
...
REM echo.
REM CHOICE /c:YN /m "%_zRED%??? Would you like to COPY the created Mod PAKs to your game folder and DELETE [DISABLEMODS.TXT] %_zDEFAULT%"
REM if %ERRORLEVEL% EQU 2 SET _bCOPYtoNMS=NONE
REM if %ERRORLEVEL% EQU 1 SET _bCOPYtoNMS=ALL
SET _bCOPYtoNMS=NONE
...
if not defined _bRecreateMapFileTrees (
	...
	echo.
	REM CHOICE /c:yn /m "%_zRED%??? Do you want to (RE)CREATE the MapFileTrees files DURING script processing %_zDEFAULT%"
	REM if %ERRORLEVEL% EQU 1 (set _bRecreateMapFileTrees=1)
	set _bRecreateMapFileTrees=1
)
...
:CONFLICTDETECTION
	...
	REM echo.
	REM CHOICE /c:yn /m "%_zRED%??? Would you like to check your NMS MODS for conflict? %_zDEFAULT%"
...
:CHECK_ExtraFilesToInclude
	...
	REM CHOICE /c:YN /m "%_zRED%??? Do you want to include them in the created PAK %_zDEFAULT%"
	REM echo.
	REM if %ERRORLEVEL% EQU 2 goto :NO_EXTRAFILES
	REM if %ERRORLEVEL% EQU 1 SET _bExtraFilesInPAK=y
...
:PAK_LISTsCREATION
	...
	REM CHOICE /c:yn /m "%_zRED%??? Do you want to RECREATE the NMS PAK file list %_zDEFAULT%"
	REM if %ERRORLEVEL% EQU 2 set _bRecreatePAKList=2
	REM if %ERRORLEVEL% EQU 1 set _bRecreatePAKList=1
